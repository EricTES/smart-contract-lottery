...
describe("VRFCoordinatorV2Mock", () => {
    ...
    let keyhash = "0xe90b7bceb6e7df5418fb78d8ee546e97c83a08bbccc01a0644d599ccd2a7c2e0"
    ...
    describe("#addConsumer", async function () {
        it("can add a consumer to a subscription", async function () {
            ...
            await expect(
                vrfCoordinatorV2Mock.connect(subOwner).addConsumer(subId, testConsumerAddress)
            )
                .to.emit(vrfCoordinatorV2Mock, "ConsumerAdded")
                .withArgs(subId, testConsumerAddress)
            let sub = await vrfCoordinatorV2Mock.connect(subOwner).getSubscription(subId)
            expect(sub.consumers).to.eql([testConsumerAddress])
        })
        it("cannot add a consumer to a nonexistent subscription", async function () {
            await expect(
                vrfCoordinatorV2Mock.connect(subOwner).addConsumer(4, testConsumerAddress)
    ...
    describe("#removeConsumer", async function () {
        it("can remove a consumer from a subscription", async function () {
            ...
            await expect(
                vrfCoordinatorV2Mock.connect(subOwner).removeConsumer(subId, testConsumerAddress)
            )
        ...
        it("cannot remove a consumer from a nonexistent subscription", async function () {
            await expect(
                vrfCoordinatorV2Mock.connect(subOwner).removeConsumer(4, testConsumerAddress)
            ).to.be.revertedWith("InvalidSubscription")
        })
    ...
    describe("#fundSubscription", async function () {
        it("can fund a subscription", async function () {
            let subId = await createSubscription()
            await expect(vrfCoordinatorV2Mock.connect(subOwner).fundSubscription(subId, oneLink))
    ...
    describe("#cancelSubscription", async function () {
        it("can cancel a subscription", async function () {
            let subId = await createSubscription()
            await expect(vrfCoordinatorV2Mock.connect(subOwner).getSubscription(subId)).to.not.be
                .reverted
    ...
    describe("#fulfillRandomWords", async function () {
        ...
        it("fails to fulfill with insufficient funds", async function () {
            ...
            await expect(
                vrfCoordinatorV2Mock.connect(random).fulfillRandomWords(1, vrfConsumerV2.address)
            ).to.be.revertedWith("InsufficientBalance")
        })
        it("can request and fulfill [ @skip-coverage ]", async function () {
            let subId = await createSubscription()
            await vrfCoordinatorV2Mock.connect(subOwner).addConsumer(subId, vrfConsumerV2.address)
            await expect(vrfCoordinatorV2Mock.connect(subOwner).fundSubscription(subId, oneLink)).to
                .not.be.reverted
            ...
            let sub = await vrfCoordinatorV2Mock.connect(random).getSubscription(subId)
            expect(sub.balance).to.equal(oneLink.sub(receipt.events[0].args["payment"]))
        })
        it("Correctly allows for user override of fulfillRandomWords [ @skip-coverage ]", async function () {
            ...
            await expect(
                vrfCoordinatorV2Mock
                    .connect(random)
                    .fulfillRandomWordsWithOverride(1, vrfConsumerV2.address, [1, 2, 3, 4, 5])
